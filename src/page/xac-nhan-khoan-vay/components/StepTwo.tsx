...
export default function StepTwo(props: Props) {
  ...
  const handleFileChange = async (
    ...
    try {
      ...
      let field = getImageField();
      setInfo((prev) => ({
        ...prev,
        [field]: res.data.imageUrl,
      }));

      // reset input để đảm bảo onChange hoạt động khi chọn lại cùng file
  ...
  const renderInputUpload = () => {
    ...
    return (
      ...
      >
        <input
          key={cccdStep} // đảm bảo input remount khi đổi step
          id="upload-cccd"
          type="file"
          hidden
          accept="image/*"
        ...
        >
          ...
          <List dense disablePadding>
            ...
            <ListItem disableGutters>
              <ListItemText
                primary="Đảm bảo thông tin rõ ràng"
                primaryTypographyProps={{ fontSize: 13 }}
              />
            </ListItem>
            <ListItem disableGutters>
